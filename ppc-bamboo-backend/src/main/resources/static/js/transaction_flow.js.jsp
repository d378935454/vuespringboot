		...
		datatables = $('#myTable').DataTable({
	       ...
	       "aoColumns" : [ {
	            ...
	            "fnRender": function (obj) {             
	            	var idlog = obj.aData["id"];
	            	return '<ppd:auth ctype="edit" menuid="73" parentid=""><a target="_blank" class="btn btn-xs btn-info btn-detail" onclick="productDetails(this,\'' + idlog + '\')">详情查看</a></ppd:auth>';
	             }
	        },],
	        "aoColumnDefs" : [ {
	            sDefaultContent : '',
	       ...
	       "fnRowCallback": function(nRow, aData, iDisplayIndex) {// 当创建了行，但还未绘制到屏幕上的时候调用，通常用于改变行的class风格 
	    	   ...
	    	   } else if (aData.supplier=='05'){
	    		   $('td:eq(10)', nRow).html("<span>宜信</span>");
	    	   } else if (aData.supplier=='06'){
	    		   $('td:eq(10)', nRow).html("<span>银联智策</span>");
	    	   } else if (aData.supplier=='07'){
	    		   $('td:eq(10)', nRow).html("<span>人人催</span>");
	    	   } else if (aData.supplier=='08'){
	    		   $('td:eq(10)', nRow).html("<span>神州融</span>");
	    	   } else if (aData.supplier=='09'){
	    		   $('td:eq(10)', nRow).html("<span>中智诚</span>");
	    	   } else if (aData.supplier=='10'){
	    		   $('td:eq(10)', nRow).html("<span>拍拍贷</span>");
	    	   } else if (aData.supplier=='11'){
	    		   $('td:eq(10)', nRow).html("<span>电话邦</span>");
	    	   } else if (aData.supplier=='12'){
	    		   $('td:eq(10)', nRow).html("<span>法海</span>");
	    	   }else {
...
function productDetails(_this,logId){
	Load('正在运行，请稍后...');
	$.post($context.ctx+"/system/creditreport/loadSerialNumberDetails?logId="+logId,function(data){
	    dispalyLoad();
		var obj = jQuery.parseJSON(data);
		var inputContentTemp = "暂无接口输入参数相关信息！";
		var outputContentTemp = "暂无接口输出参数相关信息！";
    	if(obj.resp_code == "success"){
    		var logContent = obj.resp_body.logContent;  		 
    		if (logContent !='' && logContent !=null){ 
    		 	inputContentTemp = decodeURI(logContent.textContent);
    		 	outputContentTemp = logContent.responseContent;    		 
    		} 
    	}
    	$("#modal1InterfaceRequestName").empty().html(inputContentTemp);
	 	$("#modal1InterfaceResponseName").empty().html(outputContentTemp);
	    $("#serivalNumberDetailsModal").modal({backdrop: 'static', keyboard: false}); 
	});
}
