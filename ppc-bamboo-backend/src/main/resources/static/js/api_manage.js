...
$(function(){
	function initTable(){
		datatables = $('#datatableTable').dataTable({
			...
			"sAjaxSource": $context.ctx + "/admin/api/query?type="+$("#e3").val()+"&api="+$("#e6").val(),//如果从服务器端加载数据，这个属性用语指定加载的路径
			"aoColumns" : [ {
	            "bVisible" : true,
	            "bSortable": false,
	            "mData" : "apiType",
	            ...
	            "fnRender": function (obj) {
	                ...
	                return  '<ppd:auth ctype="edit" menuid="1003" parentid="1001"><button value="'+id+'" class="btn btn-xs btn-info btn-freeze" style="margin-left:6px;" onclick="del(\'' + id + '\',\'' + apiType + '\',\'' + api + '\')">删除</button></ppd:auth>';
	                /*if (isActive == 'S') {
	                	   return '<ppd:auth ctype="edit" menuid="1003" parentid="1001"><button value="'+id+'" class="btn btn-xs btn-info btn-freeze" style="margin-left:6px;" onclick="update(\'' + id + '\',\'' + apiType + '\',\'' + api + '\')">修改</button></ppd:auth>' + 
	        ...
	        "fnRowCallback": function(nRow, aData, iDisplayIndex) {
	        	...
	        	if (aData.argumentType == '0') {
	        		$('td:eq(4)', nRow).html("<span>入参</span>");
	        	} else{
	        		$('td:eq(4)', nRow).html("<span>出参</span>");
	        	}
...
//		var searchAppkey = $("#searchAppkey").val();
		oSettings.sAjaxSource = $context.ctx + "/admin/api/query?type="+type+"&api="+api;
...
//					jAlert("保存成功","提示");//弹出对话框      					
	...
	$("#e3").click(function(){
		...
		$.ajax({
			type : "post",
			url : $context.ctx + "/admin/api/query_apiList",
			data : {type: type},
			dataType: "json",		
			success : function(data) {
	...
	$("#e1").click(function(){
		...
		$.ajax({
			type : "post",
			url : $context.ctx + "/admin/api/query_api",
			data : {type: type},
			dataType: "json",		
			success : function(data) {
				var list = data.list;
...
function returnApi(){
	window.location.href = $context.ctx + "/admin/api/api_manage";//跳转到列表页面
}

// 修改
function update(id,apiType,api){
	$.post($context.ctx+"/admin/api/update?id="+id+"&apiType="+apiType+"&api="+api,function(data){
		var obj = jQuery.parseJSON(data);
    	if(obj.resp_code == "success"){
...
function del(id,apiType,api){
	$.confirm("确定删除该用户?", function (s) {
		if (s) {$.post($context.ctx+"/admin/api/delete?id="+id+"&apiType="+apiType+"&api="+api,function(data){
				var obj = jQuery.parseJSON(data);
		    	if(obj.resp_code == "success"){
		    		//window.location.reload();
		    		datatables.fnReloadAjax(datatables.fnSettings());
