...
$(function(){
	function initTable(){
		datatables = $('#datatableTable').dataTable({
			...
			"sAjaxSource": $context.ctx + "/admin/response/query?status="+$("#e3").val(),//如果从服务器端加载数据，这个属性用语指定加载的路径
			"aoColumns" : [ {
	            "bVisible" : true,
	            "bSortable": false,
	            "mData" : "apiType",
...
//		var searchAppkey = $("#searchAppkey").val();
		oSettings.sAjaxSource = $context.ctx + "/admin/response/query?type="+type+"&api="+api;
	...
	$("#saveCallbackButton").click(function(){
		//后台交互
		...
		$.ajax({
			type : "post",
			url : $context.ctx + '/admin/response/addCallback',
			data : obj,
			async : false,
	...
	$("#e3").click(function(){
		...
		$.ajax({
			type : "post",
			url : $context.ctx + "/admin/response/query_apiList",
			data : {type: type},
			dataType: "json",		
			success : function(data) {
				...
				for(var i = 0 ; i < apiList.length ; i++){
					$("#e6").append("<option value="+apiList[i].api+">"+apiList[i].api+"</option>"); 
				}
			}
		});
	});
...
function returnBack(){
	window.location.href = $context.ctx + "/admin/response/reback_manage";//跳转到列表页面
}
