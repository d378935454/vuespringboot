var App = function () {
	...
	var handleBoxTools = function () {
		...
		jQuery('.box .tools .collapse, .box .tools .expand').click(function () {
            var el = jQuery(this).parents(".box:first").children(".box-body");
	...
	var handleBackstretch = function () {
		 $.backstretch([
		  $context.resources+"/img/login/1.jpg"
		, $context.resources+"/img/login/2.jpg"
		, $context.resources+"/img/login/3.jpg"
		, $context.resources+"/img/login/4.jpg"
	...
	var handleThemeSkins = function () {
        ...
        var setSkin = function (color) {
            $('#skin-switcher').attr("href", $context.resources+"/css/themes/" + color + ".css");
            $.cookie('skin_color', color, {expires: 7,path:'/'});//增加cookie过期时间和路径
        }
		...
		if ($.cookie('gritter_show')) {
            ...
            setTimeout(function () {
                var unique_id = $.gritter.add({
                    ...
                    image: $context.resources+'/img/gritter/cloud.png',
            ...
            setTimeout(function () {
                var unique_id = $.gritter.add({
                    ...
                    // (string | optional) the image to display on the left
                    image: $context.resources+'/img/gritter/settings.png',
                    // (bool | optional) if you want it to fade out on its own or just sit there
                    sticky: true,
                    // (int | optional) the time you want it to be alive for before fading out
                    time: '',
            ...
            setTimeout(function () {
                ...
                var unique_id = $.gritter.add({
                    ...
                    // (string | optional) the image to display on the left
                    image: $context.resources+'/img/gritter/buy.png',
                    // (bool | optional) if you want it to fade out on its own or just sit there
                    sticky: true,
                    // (int | optional) the time you want it to be alive for before fading out
                    time: '',
	...
	return {
        ...
        init: function () {
			...
			if (App.isPage("menu_manage") || App.isPage("role_manage") || App.isPage("user_manage") || App.isPage("operation_record")) {
				handleUniform();	//Function to handle uniform inputs
				handleProfileEdit(); //Function to handle profile edit tab
				handleSelect2();
			}
			if (App.isPage("interface_statistics") || App.isPage("transaction_flow") || App.isPage("data_statistics")|| App.isPage("report_manage") || App.isPage("jdCollectionMonitor")) {
				handleProfileEdit(); //Function to handle profile edit tab
				handleSelect2();
			}
			if (App.isPage("credit_report")){
				handleProfileEdit(); //Function to handle profile edit tab
				handleSelect2();
			}
        ...
        blockUI: function (el, loaderOnTop) {
            lastBlockedUI = el;
            jQuery(el).block({
                message: '<img src="'+$context.resources+'/img/loaders/12.gif" align="absmiddle">',
                css: {
                    border: 'none',
                    padding: '2px',
